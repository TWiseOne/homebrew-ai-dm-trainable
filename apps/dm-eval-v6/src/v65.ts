...
type Scenario={id:string;category:string;purpose?:string;scene:string;player:string;facts:Fact[];options:Opt[];expectedMode:"narrate"|"resolve"|"reject";expectedChoice?:string;requiredAny?:string[];forbiddenTerms?:string[];maxProposals?:number};
...
NARRATIVE AUTHORITY:
- Treat narration as candidate presentation, not permission to create facts. If a detail could be pointed at, searched, used, followed, remembered as evidence, or alter a later decision, it is factual world state, not disposable atmosphere.
- Do not invent physical properties of objects (material, carvings, locks, damage, age), architecture, possessions, relationships, or supernatural effects unless grounded in supplied facts or proposed from a flexible fact.
- A proposal is not needed for transient non-interactable texture such as general light, temperature, ordinary ambient sound, or broad smell.
- Never tell the player to roll; selecting a broker option is the complete mechanical request. The engine rolls/resolves.
...
function score(s:Scenario,r:R|null){
 ...
 const narr=low(r.narration), semantic=low(`${r.reason} ${r.narration}`);
 if(s.requiredAny?.length&&!s.requiredAny.some(x=>semantic.includes(low(x))))craft.push(`missing:${s.requiredAny.join("|")}`);
 for(const x of s.forbiddenTerms??[])if(semantic.includes(low(x)))authority.push(`unsupported:${x}`);
