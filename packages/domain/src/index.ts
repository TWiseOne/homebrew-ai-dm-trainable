...
export type GameEventType="GAME_STARTED"|"CHECK_REQUESTED"|"CHECK_RESOLVED"|"SAVE_REQUESTED"|"SAVE_RESOLVED"|"ENCOUNTER_STARTED"|"TURN_STARTED"|"TURN_ENDED"|"ATTACK_RESOLVED"|"DAMAGE_APPLIED"|"ACTOR_DEFEATED"|"ENCOUNTER_ENDED"|"SCENE_ENTERED"|"FACT_REVEALED"|"ITEM_GAINED"|"REST_COMPLETED";
export interface GameEvent<T=unknown>{id:string;type:GameEventType;at:string;payload:T}
export interface GameState { schemaVersion:1; id:string; campaignId:string; rulesetId:string; rulesVersion:string; currentSceneId:string; difficulty:GameDifficulty; dmPreferences?:DmPreferences; participants:Participant[]; controllers:Controller[]; actors:Actor[]; facts:KnowledgeFact[]; knowledge:KnowledgeGrant[]; flags:Record<string,boolean|number|string>; encounter?:EncounterState; events:GameEvent[]; }
